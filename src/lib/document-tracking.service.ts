...
export class DocumentTrackingService {
    ...
    async updateDocumentRecord(
        ...
        return this.documentUploadModel.findOneAndUpdate(
            { document_id },
            {
                ...updateDto,
                updated_at: new Date()
    ...
    async getDocumentStats(user_id?: string) {
        ...
        return this.documentUploadModel.aggregate([
            { $match: matchStage },
            {
                $group: {
                    _id: null,
    ...
    async getDuplicateReport(user_id?: string) {
        const matchStage = user_id ? { user_id, duplicate_count: { $gt: 0 } } : { duplicate_count: { $gt: 0 } };

        return this.documentUploadModel.find(matchStage)
            .select('document_id original_filename duplicate_count duplicates created_at')
            .sort({ duplicate_count: -1 })
            .exec();
    }

    async deleteDocumentRecord(document_id: string): Promise<boolean> {
        const result = await this.documentUploadModel.deleteOne({ document_id }).exec();
