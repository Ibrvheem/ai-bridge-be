...
async function main() {
  ...
  // Query for QA-approved sentences
  const query = { qa_status: 'accepted' };

  console.log('Counting QA-approved sentences...');
  const count = await Sentence.countDocuments(query);
  console.log(`Found ${count} QA-approved sentences.`);

  if (count === 0) {
    console.log('No QA-approved sentences found. Exiting.');
    await mongoose.disconnect();
    process.exit(0);
  }

  // Fetch using cursor for memory efficiency
  console.log('Fetching QA-approved sentences...');
  const sentences: any[] = [];
  ...
  const outputFile = path.join(
    exportsDir,
    `qa-approved-sentences-${timestamp}.csv`,
  );

  fs.writeFileSync(outputFile, csvContent, 'utf-8');
  console.log(
    `Exported ${sentences.length} QA-approved sentences to ${outputFile}`,
  );
